/**
 * Google Apps Script for Hiring Form Submissions
 *
 * SETUP INSTRUCTIONS:
 ...
 * 6. Copy the deployment URL and update the form action in hiring-callout.html
 */
...
const CONFIG = {
  ...
  // Sheet name where submissions will be logged
  SHEET_NAME: 'TIA Hiring',

  // Email template settings
  EMAIL_SUBJECT: 'New Hiring Application: {name}',
  FROM_NAME: 'Hiring Form',
...
/**
 * Handle POST requests from the hiring form
 */
function doPost(e) {
  try {
    // Parse form data
    ...
    if (!formData.name || !formData.email || !formData.linkedin || !formData.resume) {
    ...
    return createResponse(false, 'Error processing your submission. Please try again.');
  }
}
...
function doGet(e) {
  return HtmlService.createHtmlOutput(
    '<h1>Hiring Form Handler</h1>' +
    '<p>This endpoint accepts POST requests from the hiring form.</p>' +
    '<p>Status: Active</p>'
  );
}
...
function sendEmailNotification(data) {
  const subject = CONFIG.EMAIL_SUBJECT.replace('{name}', data.name);
